...
OpenSpec 变更名称为 `build-dining-map`，采用 `spec-driven` 工作流。目前 89 项任务中已完成 85 项，剩余 4 项均为正式发布前或真实设备验收任务。
...
- ESLint 无错误或警告。
- 生产构建通过。
- 48 个单元与组件测试通过，新增覆盖 HTML 注入、删除参与人级联、非法坐标/时间、超大文件、损坏存储、跨午夜、状态清理、历史参与人，以及草稿箱的多活动去重、恢复、删除、容量上限、损坏数据降级和 Escape 关闭。

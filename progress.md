...
- 省、市、可选区县联动选择；选择后先用高德底图原生行政区切换即时定位，再使用行政区编码与匹配级别解析中心点，空活动和已有地点的活动都会聚焦新地区。
...
- 省市区定位不再把所有请求固定为区县级别；主动选择地区会立即调用底图行政区切换并优先聚焦，容器尺寸变化也不会把视野拉回旧地点。
...
- ESLint 无错误或警告。
- 生产构建通过。
- 50 个单元与组件测试通过，新增覆盖 HTML 注入、删除参与人级联、非法坐标/时间、超大文件、损坏存储、跨午夜、状态清理、历史参与人、草稿箱，以及省市区查询级别和已有地点时的地区聚焦优先级。

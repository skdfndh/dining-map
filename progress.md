...
OpenSpec 变更名称为 `build-dining-map`，采用 `spec-driven` 工作流。目前 83 项任务中已完成 79 项，剩余 4 项均为正式发布前或真实设备验收任务。
...
- ESLint 无错误或警告。
- 生产构建通过。
- 37 个单元与组件测试通过，新增覆盖 HTML 注入、删除参与人级联、非法坐标/时间、超大文件、损坏存储、跨午夜和状态清理。
- 12 个桌面/390px 窄屏端到端场景全部通过；在线算路成功和高德服务失败后的虚线降级均作为合法且可验证的完成状态。
- 本地浏览器已验证编辑器桌面布局、参与人首次聚焦清空、键盘排序与撤销、预览 Escape 关闭，以及 390px 展示页首屏无横向溢出。
- 320px 浏览器读取被本地 URL 安全策略中止，未将该项冒充为完成；真实 iOS/Android、微信内置浏览器和横竖屏仍保留在 OpenSpec 9.2–9.4。

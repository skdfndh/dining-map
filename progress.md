...
OpenSpec 变更名称为 `build-dining-map`，采用 `spec-driven` 工作流。目前 93 项任务中已完成 89 项，剩余 4 项均为正式发布前或真实设备验收任务。
...
- ESLint 无错误或警告。
- 生产构建通过。
- 52 个单元与组件测试通过，新增覆盖 HTML 注入、删除参与人级联、非法坐标/时间、超大文件、损坏存储、跨午夜、状态清理、历史参与人、参与人聚焦面板、草稿箱，以及离线省市中心、区县回退、查询级别和已有地点时的地区聚焦优先级。
- 16 个桌面/390px 窄屏端到端场景全部通过；草稿箱自动保存、恢复与删除和历史参与人首字母筛选均纳入双视口回归，在线算路成功和高德服务失败后的虚线降级均作为合法且可验证的完成状态。
